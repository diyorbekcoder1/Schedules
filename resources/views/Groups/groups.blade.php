...
@section('content')
    ...
    <section class="no-padding-top">
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="title"><a
                        class="btn btn-primary mr-1 mb-1" href="{{route('groups.create')}}">Groups add</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                        @if(isset($posts_groups))
                            @foreach($posts_groups as $key => $data_courses)
                                <tr>
                                    <th scope="row">{{++$key}}</th>
                                    <td>{{substr($data_courses->name, 0, 20)}}</td>
                                    <td>{{$data_courses->course->name}}</td>
                                    <td>
                                <span style=" padding: 10px 20px 10px 20px; border-radius: 30px; " ;
                                      class="badge bg-{{ $data_courses->status === 1 ? 'success' : 'danger' }}">
                                    {{$data_courses->status === 1 ? 'active' : 'inactive'}}
                                </span>
                                    </td>
